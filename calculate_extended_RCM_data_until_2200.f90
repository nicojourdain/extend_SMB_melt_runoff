...
&          fidTref, Tref_ID, mtime_Tref, fidoutMLT, fidoutRUN, ki, kj, kt, ktref, state_size,          &
...
&                     file_out_smb, file_out_me, file_out_ru, file_in_Tref, &
...
REAL*4,ALLOCATABLE,DIMENSION(:,:,:) :: asmb_ref_2081_2100, ame_ref_2081_2100, aru_ref_2081_2100, tas_2081_2100
...
file_in_smb   = '/scratchu/njourdain/RCMs_PROTECT/MAR-<gcm><xx>_asmb_2015-2100_<scenar>_regrid_04000m<suffix>.nc'
file_in_me    = '/scratchu/njourdain/RCMs_PROTECT/MAR-<gcm><xx>_ame_2015-2100_<scenar>_regrid_04000m<suffix>.nc'
file_in_ru    = '/scratchu/njourdain/RCMs_PROTECT/MAR-<gcm><xx>_aru_2015-2100_<scenar>_regrid_04000m<suffix>.nc'
...
file_in_Tcmip = '/data/njourdain/DATA_PROTECT/TAS/tas_Ayr_<gcm>_<scenar>_<member>_2101_2300.nc'
file_in_Tref  = '/data/njourdain/DATA_PROTECT/TAS/tas_Ayr_<gcm>_<scenar>_<member>_2015_2100.nc'

file_out_smb  = '/scratchu/njourdain/RCMs_PROTECT/MAR-<gcm><xx>_asmb_2101-2300_<scenar>_regrid_04000m<suffix>_EXTENDED.nc'
file_out_me  = '/scratchu/njourdain/RCMs_PROTECT/MAR-<gcm><xx>_ame_2101-2300_<scenar>_regrid_04000m<suffix>_EXTENDED.nc'
file_out_ru  = '/scratchu/njourdain/RCMs_PROTECT/MAR-<gcm><xx>_aru_2101-2300_<scenar>_regrid_04000m<suffix>_EXTENDED.nc'
...
ALLOCATE(  time(mtime)  ) 
ALLOCATE(  asmb_ref_2081_2100(mx,my,20)  ) 
...
status = NF90_GET_VAR(fidinXXX,asmb_ID,asmb_ref_2081_2100,start=(/1,1,mtime-19/),count=(/mx,my,20/))
...
ALLOCATE(  ame_ref_2081_2100(mx,my,20)  ) 
 
status = NF90_INQ_VARID(fidinXXX,"ame",ame_ID); call erreur(status,.TRUE.,"inq_ame_ID")
 
status = NF90_GET_VAR(fidinXXX,ame_ID,ame_ref_2081_2100,start=(/1,1,mtime-19/),count=(/mx,my,20/))
...
ALLOCATE(  aru_ref_2081_2100(mx,my,20)  ) 
 
status = NF90_INQ_VARID(fidinXXX,"aru",aru_ID); call erreur(status,.TRUE.,"inq_aru_ID")
 
status = NF90_GET_VAR(fidinXXX,aru_ID,aru_ref_2081_2100,start=(/1,1,mtime-19/),count=(/mx,my,20/))
...
!---------------------------------------------------------
! Read 2015-2100 surface air temperature anomaly from CMIP :

write(*,*) 'Reading ', TRIM(file_in_Tref)
 
status = NF90_OPEN(TRIM(file_in_Tref),0,fidTref); call erreur(status,.TRUE.,"read")

status = NF90_INQ_DIMID(fidTref,"time",dimID_tb); call erreur(status,.TRUE.,"inq_dimID_time")
status = NF90_INQUIRE_DIMENSION(fidTref,dimID_tb,len=mtime_Tref); call erreur(status,.TRUE.,"inq_dim_time")
 
ALLOCATE(  tas_2081_2100(mx,my,20) )
 
status = NF90_INQ_VARID(fidTref,"tas",Tref_ID); call erreur(status,.TRUE.,"inq_Tref_ID")
 
status = NF90_GET_VAR(fidTref,Tref_ID,tas_2081_2100,start=(/1,1,mtime_Tref-19/),count=(/mx,my,20/)); call erreur(status,.TRUE.,"getvar_tas")
 
status = NF90_CLOSE(fidTref); call erreur(status,.TRUE.,"close_file")

!---------------------------------------------
! Read 2101-2300 CMIP surface air temperature :
...
status = NF90_GET_VAR(fidinTcmip,time_ext_ID,time_ext); call erreur(status,.TRUE.,"getvar_time")

!---------------------------------------
! Writing extended SMB over 2101-2300 :
...
status = NF90_PUT_ATT(fidoutSMB,NF90_GLOBAL,"history","Created by N. Jourdain using calculate_extended_RCM_data_until_2200.f90"); call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
...
!---------------------------------------
! Writing extended MELT over 2101-2300 :
...
status = NF90_PUT_ATT(fidoutMLT,NF90_GLOBAL,"history","Created by N. Jourdain using calculate_extended_RCM_data_until_2200.f90"); call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
...
!---------------------------------------
! Writing extended RUNOFF over 2101-2300 :
...
status = NF90_PUT_ATT(fidoutRUN,NF90_GLOBAL,"history","Created by N. Jourdain using calculate_extended_RCM_data_until_2200.f90"); call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
...
DO kt=1,mtime_ext
  ...
  DO ktref=1,20
    ...
    do kj=1,my
      ...
      if ( tas_2081_2100(ki,kj,ktref) .gt. 1.e2 .and.  tas_2081_2100(ki,kj,ktref) .lt. 350. ) then
        tas_anom(ki,kj) = tas_anom(ki,kj) - tas_2081_2100(ki,kj,ktref) ! ∆T with respect to 2100
      else
      ...
      asmb_ext(ki,kj,ktref) = (asmb_ref_2081_2100(ki,kj,ktref)+aru_ref_2081_2100(ki,kj,ktref)) * exp( coefSMB * tas_anom(ki,kj) )  &
      ...
      ame_ext(ki,kj,ktref) =  ame_ref_2081_2100(ki,kj,ktref) * exp( coefMLT * tas_anom(ki,kj) )  &
