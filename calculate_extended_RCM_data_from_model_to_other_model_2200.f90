...
! script to estimate an pseudo MAR projection for modelB over 2015-2200
! based on the existing MAR projection for modelA.
!
! **NB** You first need to run calculate_extended_RCM_climatology_from_model_to_other_model.f90
...
&          fidinTmodelB, ame_ID, aru_ID, dimID_tb,       &
&          fidinTmodelA, Tref_ID, fidoutMLT, fidoutRUN, ki, kj, kt, ame_modelA_ID, asmb_modelA_ID, aru_modelA_ID, &
&          tas_modelA_ID, fidinSMBmodelA, fidinRUmodelA, fidinMEmodelA, ktref, kr1, kr2, asmb_err_ID, ame_err_ID, aru_err_ID, &
&          fidinTmodelAhis, fidinTmodelBhis, tashis_modelA_ID, tashis_modelB_ID, lagGCM
...
rcm='MAR'
modelA='<modelA>' ; memberA='<memberA>' ; scenarA='<scenarA>'
modelB='<modelB>' ; memberB='<memberB>' ; scenarB='<scenarB>'
...
status = NF90_PUT_ATT(fidoutSMB,NF90_GLOBAL,"history","Created by N. Jourdain using calculate_extended_RCM_data_from_model_to_other_model_2200.f90")
...
status = NF90_PUT_ATT(fidoutMLT,NF90_GLOBAL,"history","Created by N. Jourdain using calculate_extended_RCM_data_from_model_to_other_model_2200.f90")
call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
...
status = NF90_PUT_ATT(fidoutRUN,NF90_GLOBAL,"history","Created by N. Jourdain using calculate_extended_RCM_data_from_model_to_other_model_2200.f90")
call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
